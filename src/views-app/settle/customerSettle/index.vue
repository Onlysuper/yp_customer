    ...
    <full-page ref="FullPage">
      ...
      <myp-loadmore-api class="list" ref="MypLoadmoreApi" :api="api" @watchDataList="watchDataList">
        <myp-cell-pannel class="spacing-20" v-for="(item,index) in list" :key="index" title="">
          <div slot="btn" v-if="(item.outMoneyStatus=='OUT_FAIL'||item.outMoneyStatus=='WAIT_AUDIT'||item.outMoneyStatus=='OUT_SEND'||item.outMoneyStatus=='AUDIT_FAIL')?true:false" @click="dataUpdate(item)">状态更新</div>
